...
def 随机分配(红包列表):
    for 索引, 当前红包金额 in 遍历索引和元素(红包列表):
        随机增减金额 = 小数(随机生成一个实数(0, 浮点数(当前红包金额 - 小数('0.01')))).quantize(小数('.00'))
        当前红包金额 -= 随机增减金额
...
较大的那批红包金额 = 小数(总金额 / 小数(红包个数)).量化(小数('.00'), 上界)
较小的那批红包金额 = 小数(总金额 / 小数(红包个数)).量化(小数('.00'), 下界)
...
# 实现Decimal求和
打印('最终红包总金额：', 合并(lambda 甲, 乙 : 甲 + 乙, 拼手气红包))
